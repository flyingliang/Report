...
namespace 班級定期評量成績單.忠信
{
    public class Program
    {
        ...
        public static void Main()
        {
            ...
            var btn = K12.Presentation.NLDPanels.Class.RibbonBarItems["資料統計"]["報表"]["成績相關報表"]["班級定期評量成績單(測試版,忠信學校)"];
            btn.Enable = false;
        ...
        //Add by HuangWC , 2013/10/2
        private static int rankDisplayDatum;
        private static Regex rgx = new Regex(".*排名[^母數].*", RegexOptions.IgnoreCase);
        ...
        private static void Program_Click(object sender_, EventArgs e_)
        {
            ...
            {
                //Add by HuangWC , 2013/10/2
                rankDisplayDatum = form.rankDisplayDatum;
                ...
                bkw.DoWork += delegate(object sender, System.ComponentModel.DoWorkEventArgs e)
                {
                    ...
                    try
                    {
                        ...
                        //Add by HuangWC , 2013/10/2
                        document.MailMerge.MergeField += new Aspose.Words.Reporting.MergeFieldEventHandler(mergeSpecial); //add by aaron ,cuz rank bigger then 20 ,they don't wanna show that
        ...
        //Add by HuangWC , 2013/10/2
        static void mergeSpecial(object sender, Aspose.Words.Reporting.MergeFieldEventArgs e)
        {
            int grade;
            if (rgx.IsMatch(e.FieldName) && int.TryParse("" + e.FieldValue, out grade))
            {
                e.Text = (grade <= rankDisplayDatum) ? "" + grade : "";
            }
        }
