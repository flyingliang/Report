    ...
    public class ExtsMore : SmartSchool.API.PlugIn.Export.Exporter
    {
        ...
        public override void InitializeExport(SmartSchool.API.PlugIn.Export.ExportWizard wizard)
        {
            ...
            wizard.ExportPackage += delegate(object sender, SmartSchool.API.PlugIn.Export.ExportPackageEventArgs e)
            {
                ...
                #region 取得及整理自訂欄位資料
                List<string> tmp_ExtsFields = e.ExportFields.FindAll(delegate(string f) { if (f.IndexOf("自訂欄位:") != -1) return true; else return false; });
                for (int i = 0; i < tmp_ExtsFields.Count; i++)
                {
        ...
        public static int SortField(string x, string y)
        {
            ...
            if (xi == -1 && yi == -1) //如果都沒有在ExportFields中則依照xy字串排序
                ...
                return ExportFields.IndexOf(x).CompareTo(ExportFields.IndexOf(y));
        }
    }
}