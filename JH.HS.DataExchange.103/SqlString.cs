...
from 
    ...
    (
        select student.id
			,case when avg(cast('0'||xpath_string('<root>'||x1.score_info||'</root>','/root/Domains/Domain[@領域=''健康與體育'']/@成績') as float)) >= 60 then '達標準' else '未達標準' end as ""健康與體育""
			,case when avg(cast('0'||xpath_string('<root>'||x1.score_info||'</root>','/root/Domains/Domain[@領域=''藝術與人文'']/@成績') as float)) >= 60 then '達標準' else '未達標準' end as ""藝術與人文""
			,case when avg(cast('0'||xpath_string('<root>'||x1.score_info||'</root>','/root/Domains/Domain[@領域=''綜合活動'']/@成績') as float)) >= 60 then '達標準' else '未達標準' end as ""綜合活動""
		from 
